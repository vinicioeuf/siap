...
import { mockRequerimentos } from "@/data/mockData";
...
const Requerimentos = () => {
  ...
  const filtered = mockRequerimentos.filter(r =>
    r.alunoNome.toLowerCase().includes(search.toLowerCase()) ||
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
              <div className="flex-1">
                <div className="flex items-center gap-3 mb-1">
                  <h3 className="text-sm font-semibold text-foreground">{req.tipo}</h3>
                  <StatusBadge status={req.status} />
                </div>
                <p className="text-xs text-muted-foreground mb-2">{req.alunoNome}</p>
                <p className="text-sm text-foreground/80">{req.descricao}</p>
              </div>
              <div className="flex flex-col items-end gap-1 text-xs text-muted-foreground shrink-0">
                <span className="flex items-center gap-1">
                  <Calendar className="h-3 w-3" />
                  Criado: {new Date(req.dataCriacao).toLocaleDateString('pt-BR')}
                </span>
                <span className="flex items-center gap-1">
                  <MessageSquare className="h-3 w-3" />
                  Atualizado: {new Date(req.dataAtualizacao).toLocaleDateString('pt-BR')}
                </span>
              </div>
            </div>
          </div>
